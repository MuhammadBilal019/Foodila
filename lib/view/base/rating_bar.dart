...
class RatingBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    ratingCount != null ? _starList.add(Padding(
      ...
      child: Text('($ratingCount)', style: muliRegular.copyWith(fontSize: size*0.8, color: Theme.of(context).disabledColor)),
    )) : SizedBox();

    return ResponsiveHelper.isWeb() ?Row(children: [
      Icon(Icons.star, color: Theme.of(context).primaryColor, size: size),
      SizedBox(width: 2,),
      Text(rating.toString(),style: muliBold.copyWith(fontSize: size*0.8,color: Theme.of(context).primaryColor),),
      SizedBox(width: 2,),
      Text('($ratingCount)', style: muliBold.copyWith(fontSize: size*0.8, color: Theme.of(context).disabledColor)),
    ],):Row(
