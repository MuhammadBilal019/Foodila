            ...
            child: Padding(
              ...
              child: GetBuilder<LocationController>(builder: (locationController) {
                return Row(crossAxisAlignment: CrossAxisAlignment.center, mainAxisAlignment: MainAxisAlignment.start,
                  ...
                  MenuButton(icon: Icons.shopping_bag, title: 'Orders'.tr, onTap: () => Get.toNamed(RouteHelper.getOrderRoute())),
                  MenuButton(icon: Icons.favorite_outlined, title: 'Favourites'.tr, onTap: () => Get.toNamed(RouteHelper.getMainRoute('favourite'))),
                      ...
                      children: [
                        GetBuilder<AuthController>(builder: (authController) {
                          return MenuButton(
                            icon: Icons.person,
                            isIcon: true,
                            title: authController.isLoggedIn() ? Images.user_icon : Images.user_icon,
                            onTap: () => Get.toNamed(authController.isLoggedIn() ? RouteHelper.getProfileRoute() : Get.dialog(WebAuthDialog())),
                          );
                        }),
                        SizedBox(width: 20,),
                        MenuButton(icon: Icons.shopping_cart, title: Images.cart,isIcon: true, isCart: true, onTap: () => Get.toNamed(RouteHelper.getCartRoute())),
                      ],
                    ),
                  ),
